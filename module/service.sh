...
enable_bypass() {
    ...
    THRESH=${THRESH:-35}
    # Method 1: MIUI/HyperOS system property
    setprop persist.vendor.battery.bypass_temp_thresh "$THRESH" 2>/dev/null || true
    resetprop persist.vendor.battery.bypass_temp_thresh "$THRESH" 2>/dev/null || true
    # Method 2: direct sysfs node (some sweet2 kernels)
    for node in /sys/class/power_supply/battery/cool_down \
                /sys/class/power_supply/battery/bypass_threshold; do
        [ -f "$node" ] && echo "$THRESH" > "$node" 2>/dev/null
    done
...
restore_bypass() {
    ...
    for node in /sys/class/power_supply/battery/cool_down \
                /sys/class/power_supply/battery/bypass_threshold; do
        [ -f "$node" ] && echo "41" > "$node" 2>/dev/null
    done
...
write_data() {
    ...
    LOG_LINE=$(tail -15 "$LOG" 2>/dev/null | tr '\n' '\x01')
    {
        echo "kernel=$KERNEL"
...
# START
# ═══════════════════════════════════════════════════════════════════════════════
log_msg "BadazZ89 k6a Optimizer v3.10 gestartet (SD730/SM7150 — sweet2, kernel 4.14 CAF)"
write_ping &
apply_balanced
write_data
